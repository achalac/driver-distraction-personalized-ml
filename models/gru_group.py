"""
GRU GROUP MODEL (LOPO) - IEEE-reviewer-safe (temporal-aware) + fast + Excel outputs
===============================================================================

This is the GROUP model using Leave-One-Participant-Out (LOPO):
Train on 49 participants, test on the held-out participant (no leakage).

Temporal + reviewer-safe properties:
✅ LOPO participant split (test participant fully held out)
...
✅ Per-fold Excel output + summary Excel across folds + loss plot PNG per fold
✅ FAST: caches per-file window tensors to disk, reused across LOPO folds
✅ Logs progress to console (epoch progress visible)

Outputs:
  <project_root>/OUTPUT_GRU_GROUP_LOPO/
  <project_root>/CACHE_WINDOWS_GRU_GROUP/
...
# ----------------------------
# Validation split (block-wise per file)
# ----------------------------
...
def save_loss_plot(out_folder, fold_id, history):
    png_path = os.path.join(out_folder, f"LOSS_GRU_LOPO_excl_{fold_id:03d}.png")
    plt.figure()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"GRU Loss (LOPO excl {fold_id:03d})")
    plt.legend()
    plt.tight_layout()
...
def save_fold_excel(out_folder, fold_id, settings, metrics_main, cm_counts, cm_norm,
    ...
    xlsx_path = os.path.join(out_folder, f"GRU_GROUP_LOPO_excl_{fold_id:03d}.xlsx")
...
def main():
    ...
    out_folder = os.path.join(project_root, "OUTPUT_GRU_GROUP_LOPO")
    cache_dir  = os.path.join(project_root, "CACHE_WINDOWS_GRU_GROUP")
    os.makedirs(out_folder, exist_ok=True)
    ...
    for exclude_pid in range(1, 51):
        t0 = time.time()
        logging.info(f"\n=== GRU LOPO fold: TEST participant {exclude_pid:03d} ===")
        ...
        logging.info("Loading/Building cached window tensors...")
        ...
        # Validation split inside training (block-wise)
    ...
    # SUMMARY Excel
    if not summary_rows:
        logging.error("No folds completed. Check your data.")
        return

    summary_df = pd.DataFrame(summary_rows).sort_values("participant")
    summary_path = os.path.join(out_folder, "SUMMARY_GRU_GROUP_LOPO.xlsx")
