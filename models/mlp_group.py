...
Cache:
  <project_root>/CACHE_WINDOWS_MLP/

Assumptions:
- Files named: GAC001_Normal_F.csv, GAC001_Load_F.csv ... up to 50
- Each CSV has >= 16 feature columns (usecols=range(16))
- Labels are file-level: Normal=0, Load=1
"""
...
def save_loss_plot(out_folder, fold_id, history):
    png_path = os.path.join(out_folder, f"LOSS_MLP_LOPO_excl_{fold_id:03d}.png")
    plt.figure()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"MLP Loss (LOPO excl {fold_id:03d})")
    plt.legend()
    plt.tight_layout()
...
def save_fold_excel(out_folder, fold_id, settings, metrics_main, cm_counts, cm_norm,
    ...
    xlsx_path = os.path.join(out_folder, f"MLP_GROUP_LOPO_excl_{fold_id:03d}.xlsx")
...
def main():
    ...
    out_folder = os.path.join(project_root, "OUTPUT_MLP_GROUP_LOPO_FAST")
    cache_dir = os.path.join(project_root, "CACHE_WINDOWS_MLP")
    os.makedirs(out_folder, exist_ok=True)
    ...
    # MLP hyperparams (transparent, stable)
    LR = 0.001
    L2 = 0.001
    H1 = 128
    H2 = 64
    ...
    # Training (GPU/CPU friendly)
    EPOCHS = 60
    BATCH = 2048          # good default; if GPU + enough windows, you can raise to 4096/8192
    VAL_FRAC = 0.10
    PATIENCE = 8

    # Shuffle-control should be lighter but valid
    SHUFFLE_EPOCHS = 10
    ...
    for exclude_pid in range(1, 51):
        t0 = time.time()
        logging.info(f"\n=== MLP LOPO fold: TEST participant {exclude_pid:03d} ===")
        ...
        Xw_train, yw_train, meta_train = concat_windows(train_items, n_features=N_FEATURES)
        Xw_test,  yw_test,  meta_test  = concat_windows(test_items,  n_features=N_FEATURES)

        logging.info(f"Windows: train={len(yw_train):,} ({dist_str(yw_train)}), test={len(yw_test):,} ({dist_str(yw_test)})")
        ...
        logging.info(f"Train: {X_tr.shape} | Val: {X_val.shape} | Balanced train: {X_tr_bal.shape} | val_dist={dist_str(y_val)}")

        # Train REAL model
        model = build_mlp(X_tr_bal.shape[1], lr=LR, l2=L2, hidden1=H1, hidden2=H2, dropout=DROPOUT)
        early = EarlyStopping(monitor="val_loss", patience=PATIENCE, restore_best_weights=True)

        logging.info("Training REAL MLP model...")
        hist = model.fit(
            X_tr_bal, y_tr_bal,
        ...
        # Shuffle negative control (lighter)
        logging.info("Training SHUFFLE control model...")
        y_tr_shuf = y_tr_bal.copy()
        np.random.shuffle(y_tr_shuf)

        model_s = build_mlp(X_tr_bal.shape[1], lr=LR, l2=L2, hidden1=H1, hidden2=H2, dropout=DROPOUT)
        model_s.fit(
            X_tr_bal, y_tr_shuf,
            validation_data=(X_val, y_val),
        ...
        # Put ALL key metrics in the summary sheet too (what you asked for)
        summary_rows.append({
    ...
    summary_path = os.path.join(out_folder, "SUMMARY_MLP_GROUP_LOPO.xlsx")
