...
Outputs:
  ...
  <project_root>/CACHE_WINDOWS_TRANSFORMER_INDIV/

Assumptions:
- Files: GAC001_Normal_F.csv, GAC001_Load_F.csv ... GAC050_*.csv
- Each CSV has at least 16 feature columns (usecols=range(16))
- Normal=0, Load=1
"""
...
def load_or_build_cached_windows(data_folder, file_id, cache_dir,
                                 fs=128, window_sec=5, n_features=16, pid=None):
    os.makedirs(cache_dir, exist_ok=True)

    if pid is None:
        # infer from filename like GAC001_...
        pid = int(file_id[3:6])
    ...
    Xw, yw, meta = build_window_tensor_from_csv(
        csv_path, file_id, pid, label, fs=fs, window_sec=window_sec, n_features=n_features
    )
...
# ----------------------------
# Temporal split (within subject)
# ----------------------------
def temporal_split_windows(X, y, meta_df, train_fraction=0.70):
    """
    Split by time order within each file separately:
      - earlier windows => train
      - later windows => test
    This is stricter than just concatenating Normal+Load and splitting globally.
    """
    train_idx, test_idx = [], []

    for file_id, g in meta_df.groupby("file_id", sort=False):
        idx = g.index.to_numpy()
        # sort by window_start_sample to preserve true time order
        ...
        test_idx.extend(idx_sorted[cut:])

    train_idx = np.asarray(train_idx, dtype=np.int64)
    test_idx = np.asarray(test_idx, dtype=np.int64)

    return X[train_idx], y[train_idx], meta_df.loc[train_idx].reset_index(drop=True), \
           X[test_idx],  y[test_idx],  meta_df.loc[test_idx].reset_index(drop=True)

# ----------------------------
# Validation split inside training (block-wise per file)
# ----------------------------
...
def build_transformer_classifier(seq_len, n_features,
                                 d_model=64, head_size=32, num_heads=2,
                                 ff_dim=128, num_blocks=2,
                                 dropout=0.2, lr=1e-3, l2=1e-4):
    inp = layers.Input(shape=(seq_len, n_features), name="window_seq")
    x = layers.Dense(d_model, kernel_regularizer=tf.keras.regularizers.l2(l2))(inp)
...
def save_loss_plot(out_folder, pid, history):
    png_path = os.path.join(out_folder, f"LOSS_TRANSFORMER_INDIV_{pid:03d}.png")
    plt.figure()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title(f"Transformer Loss (Individual {pid:03d})")
    plt.legend()
    plt.tight_layout()
...
def save_participant_excel(out_folder, pid, settings, metrics_main, cm_counts, cm_norm,
    ...
    xlsx_path = os.path.join(out_folder, f"TRANSFORMER_INDIV_{pid:03d}.xlsx")
...
def main():
    ...
    out_folder = os.path.join(project_root, "OUTPUT_TRANSFORMER_INDIVIDUAL")
    cache_dir  = os.path.join(project_root, "CACHE_WINDOWS_TRANSFORMER_INDIV")
    os.makedirs(out_folder, exist_ok=True)
    ...
    WINDOW_SEC = 5
    N_FEATURES = 16
    SEQ_LEN = FS * WINDOW_SEC  # 640

    # --- Individual temporal split ---
    TRAIN_FRACTION = 0.70  # earlier windows train, later windows test
    ...
    for pid in range(1, 51):
        t0 = time.time()
        logging.info(f"\n=== TRANSFORMER INDIV participant {pid:03d} ===")

        # Load cached windows for this participant (2 files)
        items = []
        for f in participant_files(pid):
            items.append(load_or_build_cached_windows(data_folder, f, cache_dir, FS, WINDOW_SEC, N_FEATURES, pid=pid))
        ...
        # Temporal split windows per file
        X_train, y_train, meta_train, X_test, y_test, meta_test = temporal_split_windows(
            X_all, y_all, meta_all, train_fraction=TRAIN_FRACTION
        )

        logging.info(f"Windows: train={len(y_train):,} ({dist_str(y_train)}), test={len(y_test):,} ({dist_str(y_test)})")

        if len(y_train) == 0 or len(y_test) == 0:
            logging.warning(f"Participant {pid:03d}: not enough windows for train/test. Skipping.")
            continue

        # Validation inside training (block-wise)
        ...
        # Fit scaler on TRAIN ONLY (pooled timesteps)
        ...
        model = build_transformer_classifier(
            ...
            d_model=D_MODEL, head_size=HEAD_SIZE, num_heads=NUM_HEADS,
            ff_dim=FF_DIM, num_blocks=NUM_BLOCKS,
            dropout=DROPOUT, lr=LR, l2=L2
        )
        ...
        # SHUFFLE control
        logging.info("Training SHUFFLE control Transformer (cheaper epochs)...")
        y_tr_shuf = y_tr_bal.copy()
        np.random.shuffle(y_tr_shuf)

        model_s = build_transformer_classifier(
            seq_len=SEQ_LEN, n_features=N_FEATURES,
            d_model=D_MODEL, head_size=HEAD_SIZE, num_heads=NUM_HEADS,
            ff_dim=FF_DIM, num_blocks=NUM_BLOCKS,
            dropout=DROPOUT, lr=LR, l2=L2
        ...
        settings = {
            ...
            "train_fraction_windows": TRAIN_FRACTION,
            "lr": LR,
            "l2": L2,
            "d_model": D_MODEL,
            "head_size": HEAD_SIZE,
        ...
        save_participant_excel(
            out_folder, pid, settings, metrics_main, cm, cmn,
    ...
    summary_path = os.path.join(out_folder, "SUMMARY_TRANSFORMER_INDIVIDUAL.xlsx")
